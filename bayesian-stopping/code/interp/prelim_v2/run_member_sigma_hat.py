"""Per-member driver for the sigma_hat exp-output probing sweep.

Reads hidden_cache and probe_data from a source run dir (the N=100 unified run);
writes both simple and attention-pooled probe outputs under a separate run
dir, never touching the source.
"""
...
from interp.prelim_v2.probe_sigma_hat import (
    run_sigma_hat_sweep, PROBE_TYPE_SIMPLE, PROBE_TYPE_ATTN, TARGET_NAME,
)
...
def run_member_sigma_hat(*,
    ...
    required = [
        ...
        src_member / 'probe_data' / 'X_train.npy',
    ...
    for rep in reps:
        # Idempotent: skip if outputs for both requested families already exist.
        ...
        if not (need_simple or need_attn):
            print(f'[sigma-hat member {ensemble_idx}] {rep}: outputs exist, skipping')
            continue
        families = []
        if need_simple: families.append('simple')
        if need_attn:   families.append('attn')
        t0 = time.perf_counter()
        out = run_sigma_hat_sweep(
        ...
        summary.setdefault('runs', {})[rep] = {
            ...
            'clamp_stats': {k: v['clamp_count'] for k, v in out['clamp_stats'].items()},
        }
...
def main() -> int:
    ...
    p.add_argument('--src-run-dir', type=Path, required=True,
                   help='Source run dir (N=100 unified, read-only).')
    p.add_argument('--dst-run-dir', type=Path, required=True,
                   help='New sigma_hat output run dir.')
    ...
    print(f'[run_member_sigma_hat] member {a.ensemble_idx} on {device}')
